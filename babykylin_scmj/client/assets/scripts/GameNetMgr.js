cc.Class({
    ...
    // 青阳麻将：左上角显示翻牌（带框+文字提醒）
    showLaiziView:function(){
        if(this.laizi == null || this.laizi < 0){
            return;
        }
        var self = this;
        ...
        var view = canvas.getChildByName('laizi_view');
        if(view == null){
            view = new cc.Node('laizi_view');
            view.parent = canvas;
            // 按可视区动态定位到左上角，避免不同设计分辨率下坐标出屏
            var vs = cc.winSize;
            view.setPosition(-vs.width/2 + 150, vs.height/2 - 130);
            // 插到牌桌(game)之上、结算页/弹窗遮罩等UI之下，避免翻牌压住结算和遮罩
            var gameNode = canvas.getChildByName('game');
            if(gameNode != null){
                view.setSiblingIndex(gameNode.getSiblingIndex() + 1);
            }
            // 框（半透明底+描边）
            var box = new cc.Node('box');
            box.parent = view;
            box.addComponent(cc.Graphics);
            box.zIndex = -1;
            // 翻牌麻将图
            var pai = new cc.Node('pai');
            pai.parent = view;
            pai.addComponent(cc.Sprite);
            // “翻牌”文字提醒
            var t = new cc.Node('t_title');
            t.parent = view;
            t.addComponent(cc.Label);
            t.getComponent(cc.Label).string = '翻牌';
            t.getComponent(cc.Label).fontSize = 26;
            t.color = cc.color(255, 230, 120);
        }
        try{
            var paiNode = view.getChildByName('pai');
            var mjid = this.laiziIndicator >= 0 ? this.laiziIndicator : this.laizi;
            var sf = cc.vv.mahjongmgr.getSpriteFrameByMJID('B_', mjid);
            if(sf != null){
                paiNode.getComponent(cc.Sprite).spriteFrame = sf;
            }
            // 按牌图实际尺寸画框
            var w = paiNode.width + 16;
            var h = paiNode.height + 16;
            var g = view.getChildByName('box').getComponent(cc.Graphics);
            g.clear();
            g.lineWidth = 3;
            g.strokeColor = cc.color(255, 220, 120, 255);
            g.fillColor = cc.color(0, 0, 0, 130);
            g.roundRect(-w/2, -h/2, w, h, 8);
            g.fill();
            g.stroke();
            // “翻牌”文字排在框左侧
            view.getChildByName('t_title').setPosition(-(w/2 + 32), 0);
        }catch(e){
            // 首局开局时资源可能未就绪，延迟重试兜底
            console.log('showLaiziView retry:', e);
            this.scheduleOnce(function(){ self.showLaiziView(); }, 0.5);
        }
    },
    ...
    getWanfa:function(){
        var conf = this.conf;
        if(conf && conf.type == "qingyang"){
            var strArr = ["青阳麻将"];
            if(conf.maxGames != null){
                strArr.push(conf.maxGames + "圈");
            }
            if(conf.baseScore != null){
                strArr.push("底分" + conf.baseScore);
            }
            var ruleLabels = {
                enDou:"恩豆", xiaoKai:"小开", paoFeng1:"1跑", paoFeng2:"2跑", qiXiaoDui:"七小对",
                duiDuiHu:"对对胡", quanQiuDuDiao:"全球独钓", siXi:"四喜", windArrowBonus:"风箭附加",
                wanGang:"弯杠", zhiGang:"直杠", headBonus:"头家加成", streakPenalty:"连打惩罚"
            };
            var offArr = [];
            if(conf.rules){
                for(var k in ruleLabels){
                    if(conf.rules[k] === false){
                        offArr.push(ruleLabels[k]);
                    }
                }
            }
            if(offArr.length > 0){
                strArr.push("关:" + offArr.join("、"));
            }
            return strArr.join(" ");
        }
    ...
    initHandlers:function(){
        ...
        cc.vv.net.addHandler("login_result",function(data){
            ...
            if(data.errcode === 0){
                ...
                //服务端下发已完成圈数，+1 为当前圈
                self.numOfQuans = (data.numofquans || 0) + 1;
        ...
        cc.vv.net.addHandler("game_laizi_push",function(data){
            console.log('game_laizi_push');
            self.laizi = data.laizi;
            self.laiziIndicator = data.indicator;
            self.showLaiziView();
        });
        ...
        cc.vv.net.addHandler("game_sync_push",function(data){
            ...
            self.laizi = data.laizi != null ? data.laizi : self.laizi;
            self.laiziIndicator = data.laiziIndicator != null ? data.laiziIndicator : self.laiziIndicator;
            if(self.laizi >= 0){
                self.showLaiziView();
            }
            for(var i = 0; i < 4; ++i){
        ...
        cc.vv.net.addHandler("game_quan_push",function(data){
            self.numOfQuans = data;
            self.dispatchEvent('game_quan',data);
        });
