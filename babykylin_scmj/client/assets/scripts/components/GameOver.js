cc.Class({
    ...
    onGameOver(data){
        if(cc.vv.gameNetMgr.conf.type == "qingyang"){
            this.onGameOver_QY(data);
        }
    ...
    onGameOver_QY:function(data){
        ...
        var QY_BASE_LABEL = { enDou:"恩豆", xiaoKai:"小开", paoFeng1:"1跑", paoFeng2:"2跑", qiXiaoDui:"七小对" };
        ...
        for(var i = 0; i < 4; ++i){
            ...
            // 胡牌者：青阳胡型明细（胡型、附加分、头家加成、总字数）
            var wd = userData.winDetail;
            if(wd){
                var str = userData.isZimo ? "自摸" : "接炮";
                str += "(" + (QY_BASE_LABEL[wd.baseType] || "平胡");
                if(wd.bonuses && wd.bonuses.length){
                ...
                if(wd.headBonusZi > 0){
                    str += "、头家+" + wd.headBonusZi;
                }
                str += "、" + (userData.fan || wd.totalZi) + "字";
                str += ")";
                actionArr.push(str);
            }
            else if(hued){
                // 兜底：老房间数据没有 winDetail
                var zimoWin = false;
                for(var j = 0; j < userData.actions.length; ++j){
                    if(userData.actions[j].type == "zimo") zimoWin = true;
                }
                var patStr = userData.pattern == "7pairs" ? "七小对" : (userData.pattern == "duidui" ? "对对胡" : "平胡");
                actionArr.push((zimoWin ? "自摸(" : "接炮(") + patStr + "、" + (userData.fan || 0) + "字)");
            }
            ...
            // 注意：diangangs/wangangs 是副露操作分组（大明杠/补杠），与计分无关；
            // 青阳的直杠(+10跑风)/弯杠(+4非跑风)由上面 winDetail.bonuses 按"杠时是否跑风"显示
            ...
            if(seatView.fan != null){
                seatView.fan.string = (userData.fan || 0) + "字";
            }
