    ...
    pool.getConnection(function(err,conn){  
        if(err){  
            ...
            conn.query(sql,function(qerr,vals,fields){  
                //释放连接  
                conn.release();  
                if(qerr){ console.log('db error:', qerr.message, '| sql:', String(sql).slice(0,150)); }
                //事件驱动回调（兜底：业务回调异常不再炸掉整个进程）
                try { callback(qerr,vals,fields); } catch(e) { console.log('db callback exception:', e.message); }
            });  
        }  
    });  
};
